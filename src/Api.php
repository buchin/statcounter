...
class Api
{
    public $sc_base_url = "https://api.statcounter.com";
    public $sc_version_num = "3";
    public $sc_username;
    public $sc_password;
    public $sc_query_string;
    ...
    public function get_user_project_details()
    {
        ...
        if ($xml->attributes()->status == "ok") {
            ...
            foreach ($xml->sc_data as $sc_project) {
                $result[] = [
                    "project_id" => $sc_project->project_id,
     ...
     * @throws Exception invalid credentials exception
     */
    public function create_statcounter_project(
        $website_url,
        $website_title,
        $timezone
    ) {
        ...
        $this->sc_query_string =
            "&wt=" .
            urlencode($website_title) .
            "&wu=" .
            urlencode($website_url) .
            "&tz=" .
            urlencode($timezone) .
            "&ps=0";
        ...
        if ($xml->attributes()->status == "ok") {
            ...
            foreach ($xml->sc_data as $sc_project) {
                $result = [
                    "project_id" => $xml->sc_data->project_id,
        ...
        throw new Exception(
            "Unable to create project. Check your login details."
        );
    }
     ...
     * @throws Exception
     */
    public function get_recent_keyword_activity_date_range(
        $project_id,
        $start_date,
        $end_date,
        $num_of_results = 20,
        ...
        if ($xml->attributes()->status == "ok") {
            ...
            for ($i = 0; $i < count($xml->sc_data); $i++) {
                $keyword = $xml->sc_data[$i]->keyword->__toString();

                if ($exclude_encrypted_kws) {
                    if (strpos($keyword, "Encrypted Search")) {
                        continue;
                    }
                }
     ...
     * @throws Exception
     */
    public function get_popular_pages_date_range(
        $project_id,
        $start_date,
        $end_date,
        $num_of_results = 20,
        $offset = 0,
        $count_type = "page_view"
    ) {
        $this->sc_query_string =
            "&s=popular" .
            "&pi=" .
     ...
     * @throws Exception
     */
    public function get_entry_pages_date_range(
        $project_id,
        $start_date,
        $end_date,
        $num_of_results = 20,
     ...
     * @throws Exception
     */
    public function get_exit_pages_date_range(
        $project_id,
        $start_date,
        $end_date,
        $num_of_results = 20,
        ...
        if ($xml->attributes()->status == "ok") {
            ...
            foreach ($xml->sc_data as $exit_page) {
                $result[] = [
                    "page_views" => $exit_page->page_views,
     ...
     * @throws Exception
     */
    public function get_came_from_date_range(
        $project_id,
        $start_date,
        $end_date,
        $external = 1,
     ...
     * @throws Exception
     */
    public function get_browsers_date_range(
        $project_id,
        $start_date,
        $end_date,
        $device = "all",
        ...
        if ($xml->attributes()->status == "ok") {
            ...
            foreach ($xml->sc_data as $browser) {
                $result[] = [
                    "browser_page_views" => $browser->page_views,
     ...
     * @throws Exception
     */
    public function get_operating_systems_date_range(
        $project_id,
        $start_date,
        $end_date,
        $device = "all",
    ...
    {
        return $this->get_summary_stats_date_range(
            $project_id,
            $date,
            $date
        )[0];
     ...
     * @throws Exception invalid credentials exception
     */
    public function get_summary_stats_date_range(
        $project_id,
        $start_date,
        $end_date
    ) {
        ...
        throw new Exception(
            "XML error: Check your login information and project ID."
        );
    }
     ...
     * @throws Exception invalid credentials exception
     */
    public function get_recent_visitors_date_range(
        $project_id,
        $start_date,
        $end_date,
        $num_of_results = 20,
        ...
        throw new Exception(
            "XML Error: Check your login credentials and project ID."
        );
    }
    ...
    private function build_start_end_date_url_string($start_date, $end_date)
    {
        ...
        $string =
            $string .
            "&sm=" .
            $this->date_to_array($start_date)[0] .
            "&sd=" .
            $this->date_to_array($start_date)[1] .
            "&sy=" .
            $this->date_to_array($start_date)[2] .
            "&em=" .
            $this->date_to_array($end_date)[0] .
            "&ed=" .
            $this->date_to_array($end_date)[1] .
            "&ey=" .
            $this->date_to_array($end_date)[2];
    ...
    {
        $this->sc_query_string =
            "&s=entry" . "&pi=" . $project_id . "&n=" . $num_of_results;
    ...
    {
        $this->sc_query_string =
            "&s=exit" . "&pi=" . $project_id . "&n=" . $num_of_results;
    ...
    public function get_browsers($project_id, $device = "all")
    {
        ...
        $this->sc_query_string =
            "&s=browsers" . "&de=" . $device . "&pi=" . $project_id;
    ...
    public function get_recent_pageload_activity($project_id, $device = "all")
    {
        ...
        $this->sc_query_string =
            "&s=pageload" . "&de=" . $device . "&pi=" . $project_id;
    ...
    public function get_exit_link_activity($project_id, $device = "all")
    {
        ...
        $this->sc_query_string =
            "&s=exit-link-activity" . "&de=" . $device . "&pi=" . $project_id;
